...
resource "aws_acm_certificate" "syslog" {
  domain_name       = aws_route53_record.syslog.name
  validation_method = "DNS"
  # subject_alternative_names = ["finance.internal.secondstax.com"]
  lifecycle {
    create_before_destroy = true
...
resource "aws_route53_record" "syslog-validation" {
  for_each = {
...
resource "aws_acm_certificate_validation" "syslog" {
  ...
  validation_record_fqdns = [for record in aws_route53_record.syslog-validation : record.fqdn]
